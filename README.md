...
- 入座随机头像、未开局切换头像、全桌同步
- 5 个快捷互动表情和打赏荷官
- 每手牌提供可验证公平哈希：开局公布牌堆承诺，结算后公开 seed 和完整牌牌堆
- 牌局结算记录：每手牌结束都会记录所有入座玩家的积分（开局筹码、结算后筹码、本手增减、投入与结果），实时同步给房间内所有人，并持久化到 SQLite 供战绩查询
...
房间战绩与结算记录可通过 HTTP 接口查询：`GET /api/history/rooms/:房间号` 返回参与者、手牌、打赏与每手牌的积分结算明细；`GET /api/history/rooms/:房间号/settlements` 单独返回该房间所有人的逐手积分（支持 `?limit=` 限制条数）。
